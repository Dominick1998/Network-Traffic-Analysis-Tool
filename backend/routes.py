...
@api_bp.route('/api/logs/security', methods=['GET'])
@token_required
@role_required("Admin")
def get_security_logs():
    """
    Retrieve security logs for analysis.
    Restricted to Admin role.
    """
    try:
        logs = get_logs(log_type="security")
        return jsonify(logs), 200
    except Exception as e:
        print(f"Error fetching security logs: {e}")
        return jsonify({'error': 'Unable to fetch security logs'}), 500

@api_bp.route('/api/logs/audit', methods=['GET'])
@token_required
@role_required("Admin")
def get_audit_logs():
    """
    Retrieve audit logs for user activity tracking.
    Restricted to Admin role.
    """
    try:
        logs = get_logs(log_type="audit")
        return jsonify(logs), 200
    except Exception as e:
        print(f"Error fetching audit logs: {e}")
        return jsonify({'error': 'Unable to fetch audit logs'}), 500

@api_bp.route('/api/anomalies/logs', methods=['GET'])
@token_required
@role_required("Admin")
def get_anomaly_logs():
    """
    Retrieve logs of detected anomalies.
    Restricted to Admin role.
    """
    try:
        anomaly_logs = get_anomaly_logs()
...
def get_user_activity():
    ...
    try:
        ...
        return jsonify({'error': 'Unable to fetch user activity logs'}), 500

@api_bp.route('/api/alerts/evaluate', methods=['POST'])
@token_required
@role_required("Admin")
def evaluate_alerts_manually():
    """
    Manually evaluate alerts against current network traffic data.
    Restricted to Admin role.
    """
    session = get_db_session()
    try:
        ...
        return jsonify({'error': 'Failed to evaluate alerts'}), 500
    finally:
        session.close()
...
def list_notifications():
    ...
    """
    try:
        notifications = get_notifications(user_id=request.user_id)
...
def create_user_notification():
    ...
    try:
        result = create_notification(request.user_id, message, notification_type)
        return jsonify(result), 200 if 'message' in result else 500
    except Exception as e:
...
def clear_notifications():
    ...
    """
    try:
        result = clear_notifications(user_id=request.user_id)
        return jsonify(result), 200 if 'message' in result else 500
    except Exception as e:
        print(f"Error clearing notifications: {e}")
        return jsonify({'error': 'Failed to clear notifications'}), 500

# Additional endpoints for new features can be added here.
